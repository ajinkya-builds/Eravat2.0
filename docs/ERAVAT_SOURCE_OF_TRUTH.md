...
5.  **Dashboard Configuration**: Supabase's global `Allow new users to sign up` toggle **MUST** remain enabled in the Dashboard. Disabling it completely breaks the OTP endpoint with `422 Signups not allowed for otp` even for existing users.
