...
const typeColors: Record<ObsType, string> = {
...
export default function AdminObservations() {
    ...
    const fetchObservations = async (page = 1) => {
        ...
        try {
            ...
            const { data, error, count } = await supabase
                .from('reports')
                .select('*, observations(*)', { count: 'exact' })
    ...
    const handleExportCSV = async () => {
        const { data } = await supabase.from('reports').select('*, observations(*)').order('server_created_at', { ascending: false });
        if (!data) return;
        const headers = ['ID', 'Timestamp', 'Beat', 'Type', 'Total Elephants', 'Status'];
        const rows = (data as ReportWithObs[]).map(r => {
            ...
            return [r.id, r.device_timestamp, r.beat_id, obs?.type ?? '', total, r.status].join(',');
        });
                        ...
                        <table className="w-full text-sm border-collapse">
                            <thead>
                                <tr className="border-b border-border bg-muted/30">
                                    ...
                                    {['Timestamp', 'Type', 'Elephants', 'Compass', 'Status', 'Actions'].map(h => (
                            ...
                            <tbody>
                                {observations.length === 0 ? (
                                    <tr><td colSpan={7} className="text-center py-12 text-muted-foreground">No observations yet.</td></tr>
                                    ...
                                    return (
                                        <motion.tr key={obs.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                            ...
                                            <td className="p-4 font-medium text-xs">{new Date(obs.device_timestamp).toLocaleString()}</td>
                                            <td className="p-4">
                                                {o?.type ? (
                                                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${typeColors[o.type]}`}>{o.type}</span>
                                                ) : <span className="text-muted-foreground text-xs">—</span>}
                                            </td>
                                            <td className="p-4 text-muted-foreground">{total || '—'}</td>
                                            <td className="p-4 text-muted-foreground">{o?.compass_bearing ? `${o.compass_bearing}°` : '—'}</td>
                                            <td className="p-4">
