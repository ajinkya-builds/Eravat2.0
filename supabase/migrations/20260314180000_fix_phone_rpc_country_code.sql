-- Fix get_email_by_phone to handle country codes (+91) stored in DB.
...
--
-- Also restores anon grant: login happens BEFORE authentication,
-- so unauthenticated (anon role) clients must be able to call this RPC.
...
BEGIN
  ...
  -- Normalize to last 10 digits to handle country code prefixes (e.g. 91XXXXXXXXXX → XXXXXXXXXX)
